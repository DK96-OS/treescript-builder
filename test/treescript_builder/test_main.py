...
def test_main_build_basic_tree(monkeypatch, tmp_path):
    ...
    item_list = list(map(lambda x: str(x), list(tmp_path.rglob('*'))))
    assert item_list[0].endswith('src')
    assert item_list[1].endswith(TEST_INPUT_FILE)
...
def test_main_build_empty_dirs_tree(monkeypatch, tmp_path):
    ...
    main()
    collector.assert_expected('')
    item_list = list(map(lambda x: str(x), sorted(list(tmp_path.rglob('*')))))
